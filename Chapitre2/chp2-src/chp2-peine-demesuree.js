...
function mount(root) {
    ...
    /* ── Retour vers l'openning ──
       La flèche est désormais gérée par Chapitre2Scene (ArrowChp2Part
       'peine-demesuree'), harmonisée avec le reste du site. Le module
       écoute simplement l'event global émis au clic de cette flèche. */
    on(window, 'chp2:request-return', () => {
        if (!isVideoOpen) _lancerRetourOpening();
    });
    ...
    /* ── Séquence retour vers Opening ── */
    function _lancerRetourOpening() {
        if (_aborted) return;
        /* Fade au noir de l'overlay peine-demesuree */
